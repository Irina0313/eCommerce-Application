  ...
  return (
    <>
      <Controller
        ...
        defaultValue=''
        render={({ field }) => (
          <TextField {...register(inputName, registerParams)} {...field} margin='normal' fullWidth id='email' label='Email' name='email' autoComplete='email' autoFocus error={hasError} helperText={hasError ? `⚠  ${errorsArr}` : ''} sx={{ marginTop: 0, marginBottom: 0 }} data-testid={'email'} />
        )}
      />
    </>
  );
